...
This rmd file is run by both **report.r** and **generate_reports.r** functions or it can be ran directly from the knit menu here. Make sure to type the desired runs and years on yaml.
...
all_runs <- unique(all_runs$race2)
...
caption_text2 <- paste("This table represents the total", paste(all_runs, collapse = ", "), "juvenile salmon run (s) for", paste(params$year, collapse = ", "), "brood year (s)")
...
The total catch of `r params$runs` is `r total_catch` for brood year(s) `r params$year` and a grand total of `r total_run` for all the runs `r all_runs` combined. 
